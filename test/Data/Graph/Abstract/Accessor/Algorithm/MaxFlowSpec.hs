...
edkarpSpec = describe "edkarp" $ do
    ...
    it "should work fine on graph without edges" $ do
        ...
        let flow = GAA.execute g $ do {
            source <- GAA.vfind (== 1);
            sink <- GAA.vfind (== 4);
            flow <- MaxFlow.edkarp (head source) (head sink) (const (pure 1));
    ...
    it "should be 0 for graph with edges having 0 capacity" $ do
        ...
        let (g', flow) = GAA.execute g $ do {
            source <- GAA.vfind (== 1);
            sink <- GAA.vfind (== 4);
            flow <- MaxFlow.edkarp (head source) (head sink) (const (pure 0));
            edges <- GAA.outgoing (head source);
            flow' <- traverse (GAA.eget flow) edges;
            g' <- GAA.egraph flow;
            pure (g', foldr (+) 0 flow')
    ...
    it "should be 0 for graph with source edges having 0 capacity" $ do
        ...
        let (g', flow) = GAA.execute g $ do {
            source <- GAA.vfind (== 0);
            sink <- GAA.vfind (== 3);
            flow <- MaxFlow.edkarp (head source) (head sink) (GAA.label);
            edges <- GAA.outgoing (head source);
            flow' <- traverse (GAA.eget flow) edges;
            g' <- GAA.egraph flow;
            pure (g', foldr (+) 0 flow')
    ...
    it "should correct flow in graph with cycles" $ do
